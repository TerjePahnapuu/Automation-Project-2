...
describe("Issue create", () => {
  ...
  it("Test Case 2: Random Data Plugin Issue Creation", () => {
    cy.get('[data-testid="modal:issue-create"]').within(() => {
      ...
      cy.get(".ql-editor").should("have.text", randomText);
    });

    const randomText = faker.lorem.sentence();
    cy.get('input[name="title"]').type(randomText);
    cy.get('input[name="title"]').should("have.value", randomText);
    ...
    cy.get('[data-testid="select-option:Low"]').wait(3000).trigger("mouseover").trigger("click");
